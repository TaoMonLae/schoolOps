...
router.get('/:id(\\d+)/fee-slip/pdf', requireAuth, requireRole('admin', 'teacher'), (req, res) => {
  ...
  doc.rect(0, 0, doc.page.width, 85).fill('#111827');
  const titleX = drawPdfLogo(doc, settings.logo_url, { x: 50, y: 18, size: 46 }) || 50;
  doc.fillColor('white').font('Helvetica-Bold').fontSize(18).text(settings.school_name || 'School', titleX, 24);
  doc.font('Helvetica').fontSize(11).text('Fee Payment Slip / Billing Notice', titleX, 50);
  doc.fillColor('#111827');

  let y = 108;
  doc.roundedRect(50, y, doc.page.width - 100, 180, 8).fillAndStroke('#f9fafb', '#d1d5db');
  doc.fillColor('#111827').font('Helvetica-Bold').fontSize(12).text('Student Billing Summary', 64, y + 16);
  ...
  y += 205;
  doc.roundedRect(50, y, doc.page.width - 100, 95, 8).fillAndStroke('#ffffff', '#d1d5db');
  doc.fillColor('#111827').font('Helvetica-Bold').fontSize(11).text('School Contact', 64, y + 14);
